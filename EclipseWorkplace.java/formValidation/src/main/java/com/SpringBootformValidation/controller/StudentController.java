...
//			model.addAttribute("studentDTO",new StudentDTO());
	...
	public String updatedStudent(@Valid @ModelAttribute StudentDTO studentDTO,BindingResult result, Model model, Long id) {
		if(result.hasErrors()) {
			Student student =studentrepository.findById(id).get();
			model.addAttribute("student", student);
		    return "edit-student";
		}
		studentService.updatedStudent(studentDTO,id);
		return "redirect:/";
	}
